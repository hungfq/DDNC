...
class _LoginPageState extends State<LoginPage> {
  ...
  Future _signInWithGoogle() async {
    ...
    try {
      ...
      var body = {'access_token': googleAuth?.accessToken, 'type': dropdownKey};
                ...
                Column(
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      onPressed: () async {
                        try {
                          var response = await _signInWithGoogle();
                          switch (response.base.statusCode) {
                            case 200:
                              var info = await response.body;
                              var userModel = UserModel(
                                info['userInfo']['_id'].toString(),
                                info['userInfo']['code'],
                                info['userInfo']['name'],
                                info['userInfo']['email'],
                                info['role'],
                              ...
                              Navigator.push(
                                context,
                                MaterialPageRoute(builder: (context) => const HomePage(title: "My Home Page")),
                              );
                              break;
                            default:
