...
class _AdvisorTopicListViewState extends State<AdvisorTopicListView> {
  ...
  void _handleListeners(BuildContext context, LecturerApproveState state) {
    ...
    if (state is LecturerAdvisorApprovedState) {
      var resource = state.resource;
      switch (resource.state) {
        case Result.loading:
    ...
    if (state is LecturerAdvisorDeclinedState) {
      var resource = state.resource;
      switch (resource.state) {
        case Result.loading:
          LoadingDialog.show(context);
        ...
        return AlertDialog(
          ...
          actions: [
            TextButton(
              child: Text("Approve"),
              onPressed: () {
                _approveBloc.advisorApprove(topic.id);
            ...
            TextButton(
              child: Text("Decline"),
              onPressed: () {
                _approveBloc.advisorDecline(topic.id);
                Navigator.pop(context);
              },
            ),
            TextButton(
